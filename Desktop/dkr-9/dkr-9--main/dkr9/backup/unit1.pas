...
begin
  ...
  begin
    ...
    begin
      ...
      // Если в ячейке есть текст, удаляем последний символ
      ...
      Key := 0;  // Отменяем действие по умолчанию для Backspace
    end;
  end
  else
  begin
    // Если флаг не установлен, то не разрешаем удаление
    if (Key = VK_BACK) then
    begin
      Key := 0; // Отменяем действие
    end;
  end;
end;
